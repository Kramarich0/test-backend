...
4. **Heartbeat терминалов**:
   - `POST /terminals/alive` принимает физический `macAddress` оборудования и обновляет статус терминала в `ACTIVE` (при записи автоматически обновляется и `updatedAt`). Касса идентифицируется по аппаратному MAC-адресу из тела запроса, а не по внутреннему UUID в пути (в отличие от остальных ручек `/terminals/:id`).
...
5. **Безопасность паролей (OWASP + защита от Bcrypt DoS)**:
   ...
   - Вход через `LoginDto` всегда отвечает одинаковым `401 Invalid Password or Email` — и для несуществующего email, и для неверного пароля, поэтому перебор пользователей по ответу API невозможен.
6. **Fail-Fast валидация окружения**:
   - При старте `ConfigModule` строго валидирует тип и диапазон порта, обязательность `DATABASE_URL` и формат JWT-таймаутов (`JWT_EXPIRES_IN`, `JWT_REFRESH_EXPIRES_IN`) через `class-validator` / `class-transformer`.
7. **Shared Kernel для общих DTO**:
   ...
   - Это исключает дублирование идентичных полей DTO без появления запрещённых кросс-модульных импортов: правило архитектуры `vsa-no-cross-slice-imports` запрещает заходить во внутренности чужого модуля (DTO, контроллеры, прочие файлы), но разрешает импорт из общего ядра `common`, а также регистрацию модулей и внедрение их экспортируемых сервисов через DI (например, `ProfileModule` → `AdminsModule`).
...
> - Все параметры пути `/:id` должны быть валидными UUID — иначе ответ `400`.
