...
const config: IConfiguration = {
  forbidden: [
    ...
    {
      comment:
        "This module depends on a spec (test) file. The responsibility of a spec file is to test code. If there's something in a spec that's of use to other modules, it doesn't have that single responsibility anymore. Factor it out into (e.g.) a separate utility/ helper or a mock.",
      from: {},
      name: 'not-to-spec',
      severity: 'error',
      to: {
        path: '[.](?:spec|e2e-spec|test)[.](?:js|mjs|cjs|jsx|ts|mts|cts|tsx)$',
      },
    },
    {
      comment:
      ...
      from: {
        path: '^(src)',
        pathNot: '[.](?:spec|e2e-spec|test)[.](?:js|mjs|cjs|jsx|ts|mts|cts|tsx)$',
      },
      name: 'not-to-dev-dep',
      severity: 'error',
      to: {
    ...
    {
      name: 'service-not-to-controller',
      comment: 'NestJS: Services should never depend on Controllers (layers must point down).',
      severity: 'error',
      from: {
    ...
    },
    {
      name: 'dto-pure',
      comment:
        'NestJS: DTOs must be pure data structures and not depend on services, controllers or database.',
      severity: 'error',
      from: {
        path: '^src/modules/[^/]+/dto/',
      },
      to: {
        path: ['[.]service[.]ts$', '[.]controller[.]ts$', '^src/db/'],
      },
    },
    {
      comment:
    ...
    {
      comment:
        'VSA (test-backend): module unreachable from src/main.ts — dead code or unmounted module.',
      from: {
        path: '^src/main[.]ts$',
      },
  ...
  options: {
    ...
    webpackConfig: {
      fileName: 'dependency-cruiser.webpack.config.cjs',
    },
  },
};
